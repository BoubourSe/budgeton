...
class Conf:
    ...
    def open_file(self):
        ...
        if self._f != None and self._f.closed == False :
            pass

        try:
            # open for read
            self._f = open(self._directory.joinpath(self._name), 'r+', encoding='utf-8')
        except FileNotFoundError:
            # create if not exist
            self._f = open(self._directory.joinpath(self._name), 'w', encoding='utf-8')
            created = True
        finally:
            if created :
                self._f = open(self._directory.joinpath(self._name), 'r+', encoding='utf-8')
